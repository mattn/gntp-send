...
void
md5_process(md5_context *ctx, const uint8_t data[64]) {
...
void
md5_update(md5_context *ctx, const uint8_t *input, uint32_t length) {
  uint32_t left, fill;
...
void
md5_finish(md5_context *ctx, uint8_t digest[16]) {
  uint32_t last, padn;
  uint32_t high, low;
  uint8_t msglen[8];
